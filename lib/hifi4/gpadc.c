...
void gpadc_init(uint32_t sample_rate)
{
    ...
    reg_val |= GPADC_CTRL_WORK_MODE_SINGLE;
    gpadc_write_reg(GPADC_CTRL, reg_val);
}
...
bool gpadc_is_channel_enabled(gpadc_channel_t channel)
{
    ...
    return (cs_en & GPADC_CS_EN_CH(channel)) != 0;
}
...
void gpadc_channel_enable(gpadc_channel_t channel)
{
    ...
    uint32_t reg_val = gpadc_read_reg(GPADC_CTRL);
...
bool gpadc_is_channel_ready(gpadc_channel_t channel)
{
    ...
    return (status & GPADC_DATA_INTS_CH(channel)) != 0;
}
